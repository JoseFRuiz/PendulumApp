...
def find_next_crossing(angles, start_idx, window, target,
                        direction=None, direction_eps=3.0, extreme_deg=None,
                        immediate_eps=0.5):
    """Search angles[start_idx : start_idx+window] (clamped to len(angles)-1) for
    where the signal crosses `target`. Returns a fractional index (float) for the
    interpolated crossing, or None if no qualifying crossing exists in the window.

    If angles[start_idx] is already within `immediate_eps` of target, start_idx is
    returned directly (zero position error). `direction` is the sign of the
    target's own instantaneous derivative (e.g. cos(2*pi*t/period)); a candidate
    crossing only counts if the real signal's local slope has the same sign, unless
    abs(target) >= extreme_deg - direction_eps, where direction is ambiguous near
    the amplitude extremes and the filter is skipped. None entries in `angles` are
    skipped rather than treated as crossings (defensive; fill_gaps output normally
    has none, except when there was never a valid detection anywhere).
    """
    n = len(angles)
    ...
    a0 = angles[start_idx]
    if a0 is not None and abs(a0 - target) <= immediate_eps:
        return float(start_idx)

    skip_direction = direction is None or (
        extreme_deg is not None and abs(target) >= extreme_deg - direction_eps)
    ...
    for j in range(start_idx, end_idx):
        ...
        if a is None or b is None:
            continue
        if (a - target) == 0:
            continue  # already handled as an immediate crossing when j == start_idx
        if (a - target) * (b - target) > 0:
            continue  # no crossing in [j, j+1]

        slope = b - a
        if not skip_direction and direction != 0 and slope * direction < 0:
            continue

        frac = (target - a) / slope if slope != 0 else 0.0
        return j + max(0.0, min(1.0, frac))
